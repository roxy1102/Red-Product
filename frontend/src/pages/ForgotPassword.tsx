...
// Définir le type de retour attendu de forgotPassword
...
const ForgotPassword: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setMessage(response.message || "Lien envoyé (si cet email existe).");
      setError("");
    } catch (err: unknown) {
      // gérer correctement l'erreur axios
      if (err && typeof err === 'object' && 'response' in err) {
        // typer la réponse d'erreur
      ...
      <div className="relative z-10 flex flex-col items-center w-full max-w-md p-4">
        ...
        <form onSubmit={handleSubmit} className="p-4 bg-white shadow-lg w-full">
          ...
          <h3 className="text-center mb-6 text-gray-600 text-sm sm:text-base">
            Entrez votre adresse e-mail ci-dessous et nous vous envoyons des instructions.
          </h3>
          ...
          <button type="submit" className="w-full bg-gray-800 text-white p-2 rounded hover:bg-blue-600 transition">
            Envoyer le lien
          </button>
        </form>
